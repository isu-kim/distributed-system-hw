...
func Parse(filePath string) (error, Config) {
	...
	if err != nil {
		fmt.Printf("[ERROR] Unable to read the config file %s: %v", filePath, err)
		return err, Config{}
	}
	...
	if err != nil {
		fmt.Printf("[ERROR] Unable to unmarshall the config file %s: %v", filePath, err)
		return err, Config{}
	}
	...
	err = config.isValid()
	if err != nil {
		fmt.Printf("[ERROR] Unable to load config file properly: %v", err)
		return err, Config{}
	}
...
func (c Config) PrintConfig() {
	fmt.Printf("SyncType: %s\n", c.Sync)
	c.PrintReplica()
}

// PrintReplica prints out the replicas in the config
func (c Config) PrintReplica() {
